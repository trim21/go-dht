...
import (
	...
	"github.com/trim21/go-dht/internal/nettools"
)
...
func (r *routingTable) hostPortToNode(hostPort string, network string) (node *remoteNode, addr string, existed bool, err error) {
	if hostPort == "" {
		panic("programming error: hostPortToNode received a nil hostPort")
	}

	address, err := net.ResolveUDPAddr(network, hostPort)
	if err != nil {
